// ⭐ UPDATED: Create session only when requesting hints (with auto voice - no auth for voice)
    ...
    if (hintText) {
      try {
        ...
        if (voiceRes.ok) {
          ...
          // Handle voice service errors gracefully
          console.log('Voice service error:', voiceRes.status);
          if (voiceRes.status === 503) {
            console.log('Voice service not configured - hints will be text only');
          }
        }
      } catch (voiceError) {
