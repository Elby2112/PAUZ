// ⭐ UPDATED: Create session only when requesting hints (with auto voice + auth check)
    ...
    if (hintText) {
      try {
        ...
        if (voiceRes.ok) {
          ...
          console.log('Voice service not available:', voiceRes.status);
          // Continue showing hint without voice
        }
      } catch (voiceError) {
